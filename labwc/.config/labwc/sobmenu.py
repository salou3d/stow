...
def get_installed_terminal(apps):
    for app in apps:
        iterms = {}
        if apps[app].getGenericName() == 'Terminal':
            iterms[ apps[app].getExec() ] = apps[app].getGenericName()
    if iterms:
        if TERMINAL in iterms:
            return TERMINAL
        else:
            return list(iterms.keys())[0]
    else:
        return environ["TERM"]
...
def main():
    ...
    if soargs.menu:
        ...
        TERMINAL = get_installed_terminal(apps_dict)
    ...
    if not soargs.menu:
        if pipe:
            ...
            ###
            cats_dict, apps_dict, theme = prepare_dicts( useicons )
            ET.dump( build_apps_menu( cats_dict, apps_dict, theme, pipe, useicons ))
